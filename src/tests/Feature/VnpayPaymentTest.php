...
class VnpayPaymentTest extends TestCase
{
    ...
    public function test_customer_can_create_gateway_payment()
    {
        ...
        $response->assertStatus(200)
            ...
            ->assertJsonStructure([
                'data' => [
                    'id', 'job_id', 'amount', 'payment_method',
                    'gateway_order_id', 'pay_url', 'status',
                ],
            ]);
    ...
    public function test_vnpay_ipn_invalid_amount()
    {
        $job = Job::factory()->create([
            'customer_id' => $this->customer->id,
            'status' => JobStatusConst::PENDING_PAYMENT,
        ...
        $inputData = [
            'vnp_Amount' => 500000, // Wrong amount
            'vnp_ResponseCode' => '00',
            'vnp_TxnRef' => 'GATEWAY456',
        ];

        $hashdata = 'vnp_Amount=500000&vnp_ResponseCode=00&vnp_TxnRef=GATEWAY456';
        $signature = hash_hmac('sha512', $hashdata, 'TESTHASHSECRET');
        $inputData['vnp_SecureHash'] = $signature;
    ...
    public function test_vnpay_ipn_already_paid()
    {
        $job = Job::factory()->create([
            'customer_id' => $this->customer->id,
            'status' => JobStatusConst::PAID,
        ...
        $payment = Payment::create([
            ...
            'code' => 'PAY12345679',
            'payment_method' => 'VNPAY',
            'status' => PaymentStatusConst::PAID, // Already paid
            'gateway_order_id' => 'GATEWAY456',
        ]);
        ...
        $hashdata = 'vnp_Amount=11000000&vnp_ResponseCode=00&vnp_TxnRef=GATEWAY456';
        $signature = hash_hmac('sha512', $hashdata, 'TESTHASHSECRET');
        $inputData['vnp_SecureHash'] = $signature;
